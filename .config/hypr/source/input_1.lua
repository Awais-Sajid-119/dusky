...
-- -------------------------------------------------------------------------------------------------
-- 3. SCROLLING & TRACKBALLS (Renumbered based on your header)
-- -------------------------------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------------------------------
-- 7. TABLET CONFIGURATION (Preserved as Lua comment structure)
-- -------------------------------------------------------------------------------------------------
    -- device = {
...
-- -------------------------------------------------------------------------------------------------
-- 7. GESTURE BINDINGS (Mapped to a Lua array)
-- -------------------------------------------------------------------------------------------------
    gesture = {
        -- 3-finger up swipe -> hyprexpo-toggle
        [[3, up, dispatcher, hyprexpo:expo, toggle]],
        -- 3-finger left swipe -> 
        [[3, left, dispatcher, exec, ]],

        -- --- Regular toggeling without osdclient ---

        -- 3-finger down/right swipe -> mute/pause-play
        [[3, down, dispatcher, exec, wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle]],
        [[3, right, dispatcher, exec, playerctl play-pause]],

        -- 4-finger horizontal swipe -> brightness change
        [[4, left, dispatcher, exec, brightnessctl -e4 -n2 set 10%-]],
        [[4, right, dispatcher, exec, brightnessctl -e4 -n2 set 10%+]],

        -- 4-finger vertical swipe -> volume change
        [[4, up, dispatcher, exec, wpctl set-volume -l 1.5 @DEFAULT_AUDIO_SINK@ 10%+]],
        [[4, down, dispatcher, exec, wpctl set-volume @DEFAULT_AUDIO_SINK@ 10%-]],

        -- ---

        -- 3-finger pinch in -> Lock screen
        [[3, pinchout, dispatcher, exec, hyprlock --immediate]],
        -- 3-finger pinch out -> screenshot
        [[3, pinchin, dispatcher, exec, slurp | grim -g - - | swappy -f -]],
        -- 2-finger pinch out -> 
        [[4, pinchout, dispatcher, exec,]]
    }
})
