...
readonly PIN_READ_MAX=10000
readonly PREVIEW_TEXT_LIMIT=50000

# Critical: Forced to C.UTF-8 to prevent byte-level truncation of multibyte 
# characters (like emoji) during awk string slicing and read -n operations.
readonly TEXT_LOCALE="C.UTF-8"
readonly LIST_LOCALE="C.UTF-8"
...
# Modifies tracker directly in scope via nameref to prevent subshell memory leaks
make_tmpfile() {
    local dir="$1" template="${2:-.tmp.XXXXXX}"
    local -n _out_tmp_ref="$3"
    dir_ready "$dir" || ensure_private_dir "$dir" || return 1
    _out_tmp_ref=$(mktemp "${dir%/}/${template}") || return 1
    _TMPFILES+=("$_out_tmp_ref")
    return 0
}
...
# Prevents subshells by resolving state dynamically through nameref variables
decode_entry_to_tmp() {
    local id="$1" dir="$2" template="${3:-.tmp.XXXXXX}"
    local -n _out_file_ref="$4"
    make_tmpfile "$dir" "$template" _out_file_ref || return 1
    if cliphist_decode_to_file "$id" "$_out_file_ref"; then
        return 0
    fi
    remove_tmpfile "$_out_file_ref"
    _out_file_ref=""
    return 1
}
...
# Parse cleanly. Used for final copy commands where the full line is accepted directly.
parse_item() {
    local input="$1"
...
# Zero-fork system process reading via bash native inputs mapped directly to namerefs
proc_comm() {
    local pid="$1"
    local -n _out_comm="$2"
    [[ -r "/proc/$pid/comm" ]] || return 1
    IFS= read -r _out_comm < "/proc/$pid/comm" || [[ -n "$_out_comm" ]] || return 1
    return 0
}
...
cache_image() {
    local id="$1"
    local -n _out_cache_ref="$2"
    local tmp="" mime=""

    is_uint "$id" || return 1

    if find_cached_image "$id" _out_cache_ref; then
        return 0
    fi

    decode_entry_to_tmp "$id" "$CACHE_DIR" ".img.XXXXXX" tmp || return 1
    ...
    _out_cache_ref="$CACHE_DIR/${id}.img"
    if mv -f -- "$tmp" "$_out_cache_ref" 2>/dev/null; then
        untrack_tmpfile "$tmp"
        return 0
    fi
...
copy_binary_entry() {
    ...
    decode_entry_to_tmp "$id" "$CACHE_DIR" ".tmp.XXXXXX" tmp || return 1
...
#==============================================================================
# TIME FORMATTING ENGINE (Zero-Fork Optimization)
#==============================================================================
get_formatted_time() {
    local type="$1"
    local id="$2"
    ...
    if [[ "$type" == "pin" && -n "$pin_file" && -f "$pin_file" ]]; then
        ts=$(stat -c %Y "$pin_file" 2>/dev/null)
    else
        # Infer timestamp bounds internally if present (for modern cliphist DBs)
        if is_uint "$id"; then
            if (( id > 1000000000 && id < 3000000000 )); then
                ts="$id"
            elif (( id > 1000000000000 && id < 3000000000000 )); then
                ts=$(( id / 1000 ))
            elif (( id > 1000000000000000 && id < 3000000000000000 )); then
                ts=$(( id / 1000000 ))
            fi
        fi
    fi
    ...
    # Pure Bash 5.0+ zero-fork native time formatting
    printf -v now '%(%s)T' -1
    week_ago=$(( now - 604800 ))
    ...
    # %-I suppresses the leading zero for single digit hours
    printf -v time_str '%(%-I:%M %p)T' "$ts"

    if (( ts >= week_ago )); then
        printf -v _out_ref '%s %s' "$day_name" "$time_str"
    else
        printf -v date_str '%(%m/%d)T' "$ts"
        printf -v _out_ref '%s %s %s' "$date_str" "$day_name" "$time_str"
    fi
    return 0
}
...
cmd_list() {
    local n=0
    local pin hash preview full_search
    ...
    while IFS= read -r pin; do
        ...
        full_search=""
        LC_ALL="$TEXT_LOCALE" IFS= read -r -d '' -n "$PIN_READ_MAX" full_search < "$pin" || true
        [[ -z "$full_search" ]] && continue

        # Format full payload safely for the hidden FZF search column
        full_search="${full_search//$'\n'/ }"
        full_search="${full_search//$'\r'/}"
        full_search="${full_search//$'\t'/ }"
        full_search="${full_search//"$SEP"/ }"

        preview="$full_search"
        # Zero-fork whitespace squashing natively in Bash
        ...
        printf '%d %s %s%s%s%s%s%s%s\n' \
            "$n" "$ICON_PIN" "$preview" "$SEP" "pin" "$SEP" "$hash" "$SEP" "$full_search"
    done < <(
...

    {
        ...
        if (content ~ /^\[\[ *binary data/) {
            ...
            if (dims != "" || fmt != "") {
                ...
                if (info == "") info = "Image"
                printf "%d %s %s%s%s%s%s%s%s\n", idx, icon_img, info, sep, "img", sep, id, sep, info
            } else {
                info = content
                ...
                printf "%d %s %s%s%s%s%s%s%s\n", idx, icon_bin, info, sep, "bin", sep, id, sep, info
            }
        } else {
            orig_content = content
            gsub(/[[:cntrl:]]/, " ", orig_content)
            gsub(sep, " ", orig_content)

            display_content = orig_content
            gsub(/  +/, " ", display_content)
            gsub(/^ +| +$/, "", display_content)

            if (display_content == "") display_content = "[Whitespace]"
            if (length(display_content) > max_len) display_content = substr(display_content, 1, max_len) "…"

            printf "%d %s%s%s%s%s%s%s\n", idx, display_content, sep, "txt", sep, id, sep, orig_content
        }
    }
...
preview_binary_entry() {
    ...
    decode_entry_to_tmp "$id" "$CACHE_DIR" ".tmp.XXXXXX" tmp || {
...
cmd_preview() {
    local type="${1:-}" id="${2:-}" pin_file img_path info tmp=""
    ...
    case "$type" in
        ...
        img)
            ...
            printf '\e[1;36m━━━ %s IMAGE ━━━\e[0m\n' "$ICON_IMG"
            if cache_image "$id" img_path; then
    ...
    # Retrieve formatted timestamp completely fork-free via bash pass-by-reference
    local footer_time=""
    case "$type" in
        pin) get_formatted_time "pin" "$id" "${PINS_DIR:?}/${id}.pin" footer_time ;;
        img|bin|txt) get_formatted_time "$type" "$id" "" footer_time ;;
    esac

    if [[ -n "$footer_time" ]]; then
        printf '\n\e[90m──────────────────────────────────────────\e[0m\n'
        printf '\e[36m🕒 %s\e[0m\n' "$footer_time"
    fi
}
...
cmd_pin() {
    local type="${1:-}" id="${2:-}" pin_file hash tmp=""
    ...
    case "$type" in
        ...
        txt)
            is_uint "$id" || return 1
            decode_entry_to_tmp "$id" "$PINS_DIR" ".pin.XXXXXX" tmp || return 1
            ...
            ;;
        img|bin)
            notify "Pinning images and binaries is not supported yet." "low"
            return 1
            ;;
        *)
            return 0
...
cmd_delete() {
    ...
    case "$type" in
        ...
        img|bin)
            is_uint "$id" || return 1
            cliphist_feed_id "$id" | cliphist delete 2>/dev/null || status=$?
...
show_menu() {
    ...
    # Zero loop, perfectly secure argument escaping
    printf -v self_q '%q' "$SELF"

    # NOTE: By isolating {2} and {3} (type and ID), we completely evade the Linux
    # ARG_MAX buffer limit, preventing the script from crashing with E2BIG when 
    # executing background actions on multi-megabyte clipboard text payloads.
    selection=$(
        cmd_list | fzf \
            --extended --tiebreak=index \
            ...
            --prompt="  " --pointer="▌" --delimiter="$SEP" --with-nth=1 --nth=1,4 \
            --preview="${self_q} --preview '{2}' '{3}'" --preview-window="right,45%,~1,wrap" \
            --bind="enter:accept" \
            --bind="alt-u:execute-silent(${self_q} --pin '{2}' '{3}')+reload(${self_q} --list)" \
            --bind="alt-y:execute-silent(${self_q} --delete '{2}' '{3}')+reload(${self_q} --list)" \
